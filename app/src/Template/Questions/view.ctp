    ...
    <?php else: ?>
        ...
        <?php foreach ($answers as $answer): ?>
            <div class="card w-75 mb-2 ml-auto">
                <div class="card-body">
                    <h5 class="card-title">
                        <i class="fas fa-user-circle"></i> <?= 'じろう' // @TODO ユーザ管理機能実装時に修正 ?>
                    </h5>
                    <p class="card-text"> <?= nl2br(h($answer->body)) ?></p>
                    <p class="card-subtitle mb-2 text-muted">
                        <small> <?= $answer->created ?></small>
                        <?= $this->Form->postlink('削除する', ['controller' => 'Answers', 'action' => 'delete', $answer->id], ['confirm' => '回答を削除します。よろしいですか?'], ['class' => 'card-link']) ?>
                    </p>
                </div>
            </div>
        <?php endforeach; ?>
...
<section class="answer-post mb-5">
    ...
    <?php if ($answers->count() >= 100): ?>
        <p class="text-center">回答数が上限に達しているため回答することはできません</p>
    <?php else: ?>
